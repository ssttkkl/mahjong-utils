...
fun shanten(tiles: List<Tile>, furo: List<Furo> = emptyList(), calcAdvanceNum: Boolean = true): ShantenResult {
    ...
    var shantenInfo = if (!withGot) {
        ...
        if (regular.shantenInfo.shantenNum == shantenNum) {
            mergeIntoWithoutGot(advance, wellShapeAdvance, patterns, regular)
        }
        if (chitoi.shantenInfo.shantenNum == shantenNum) {
            mergeIntoWithoutGot(advance, wellShapeAdvance, patterns, chitoi)
        }
        if (kokushi.shantenInfo.shantenNum == shantenNum) {
        ...
        mergeIntoWithGot(discardToAdvance, patterns, regular)
        mergeIntoWithGot(discardToAdvance, patterns, chitoi)
        mergeIntoWithGot(discardToAdvance, patterns, kokushi)
